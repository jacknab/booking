  ...
  return (
    <div className="font-sans text-gray-800 overflow-x-hidden">
      ...
      >
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
          <button onClick={() => scrollTo('hero')} className="flex items-center gap-2.5">
            <div className="w-10 h-10 rounded-full bg-amber-500 flex items-center justify-center shadow-md">
              <Star className="w-5 h-5 text-gray-900 fill-amber-500" />
            </div>
            <span
              className={`text-lg font-bold tracking-wide transition-colors duration-300 ${
                scrolled ? 'text-amber-400' : 'text-amber-300 drop-shadow'
              }`}
          ...
          <div className="hidden md:flex items-center gap-8">
            {[{ label: 'HOME', id: 'hero' }, { label: 'ABOUT US', id: 'about' }, { label: 'GALLERY', id: 'gallery' }, { label: 'SERVICES', id: 'services' }, { label: 'CONTACT US', id: 'contact' }].map(({ label, id }) => (
              <button
                key={id}
                onClick={() => scrollTo(id)}
                className={`text-xs font-semibold tracking-widest transition-colors duration-300 hover:text-amber-300 ${
                  scrolled ? 'text-amber-300' : 'text-gray-100 drop-shadow'
                }`}
              >
                {label}
              </button>
            ...
            <button
              ...
              className="px-5 py-2 bg-amber-500 hover:bg-amber-600 text-gray-900 text-xs font-bold tracking-wider rounded border-2 border-amber-500 transition-colors duration-200 shadow"
            >
              APPOINTMENT
            </button>
          </div>
          ...
          <button
            className={`md:hidden transition-colors duration-300 ${
              scrolled ? 'text-amber-300' : 'text-white'
            }`}
        ...
        {menuOpen && (
          <div className="md:hidden bg-gray-900 border-t border-amber-500/30 shadow-lg">
            {[{ label: 'HOME', id: 'hero' }, { label: 'ABOUT US', id: 'about' }, { label: 'GALLERY', id: 'gallery' }, { label: 'SERVICES', id: 'services' }, { label: 'CONTACT US', id: 'contact' }].map(({ label, id }) => (
              <button
                key={id}
                onClick={() => scrollTo(id)}
                className="block w-full text-left px-6 py-4 text-sm font-semibold uppercase tracking-widest text-amber-300 hover:bg-amber-500/10 hover:text-amber-200 transition-colors"
              >
                {label}
              </button>
            ))}
      ...
      <section id="hero" className="relative h-screen w-full overflow-hidden">
        ...
        <div className="relative z-20 h-full flex flex-col items-start justify-center text-white px-6 sm:px-12 max-w-7xl">
          <p className="text-amber-300 text-sm font-semibold tracking-widest mb-3 animate-fadein">
            Luxury Nails, Ultimate Comfort
          </p>
          <h1
            className="text-5xl md:text-7xl font-light leading-tight mb-8 animate-fadein max-w-2xl"
            style={{ textShadow: '0 2px 24px rgba(0,0,0,0.6)', fontFamily: 'Georgia, serif', fontWeight: 300 }}
          >
            Luxury Nails Spa
          </h1>
          <div className="flex flex-col sm:flex-row gap-4 animate-fadein">
            <button
              onClick={() => scrollTo('contact')}
              className="px-6 py-2.5 bg-amber-500 hover:bg-amber-600 text-gray-900 font-bold text-sm tracking-wider border-2 border-amber-500 transition-all duration-200 rounded shadow-lg"
            >
              CALL NOW
            </button>
            <button
        ...
        <div className="absolute bottom-16 right-6 z-20 flex flex-col gap-4">
          <button
            onClick={() => scrollTo('contact')}
            className="w-12 h-12 bg-amber-500 hover:bg-amber-600 rounded-full flex items-center justify-center shadow-lg transition-all duration-200 hover:scale-110"
            title="Book appointment"
          >
            <Calendar className="w-6 h-6 text-gray-900" />
          </button>
          <button
            onClick={() => window.location.href = 'tel:+13036901099'}
            className="w-12 h-12 bg-amber-500 hover:bg-amber-600 rounded-full flex items-center justify-center shadow-lg transition-all duration-200 hover:scale-110"
            title="Call us"
          >
        ...
        <div className="absolute bottom-16 right-24 z-20 flex gap-2">
          <button
            onClick={() => triggerSlide((currentSlide - 1 + heroImages.length) % heroImages.length)}
            className="w-10 h-10 bg-white/20 hover:bg-white/40 rounded text-white transition-all duration-200"
          >
            ←
          </button>
          <button
            onClick={() => triggerSlide((currentSlide + 1) % heroImages.length)}
            className="w-10 h-10 bg-white/20 hover:bg-white/40 rounded text-white transition-all duration-200"
          >
            →
          </button>
