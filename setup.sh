...
domain instead of using their own subdomain.)" \
    ...
    # X-Forwarded-Host is passed on every block so Express can read the
    # original hostname if needed in the future.
...
#  server_name includes ${DOMAIN} and www.${DOMAIN} for proper CORS handling.
#  www.${DOMAIN} is redirected to ${DOMAIN} to avoid CORS issues.
#  X-Forwarded-Host is passed on every proxy block so Express can read the
#  original hostname if needed in the future.
#
...
server {
    ...
    # Root domain only (www is redirected above)
    server_name ${DOMAIN};
