...
import { ArrowRight, Calendar, Users, DollarSign, PlayCircle, Smartphone, Star, ShieldCheck, MapPin, Repeat, CheckCircle, BarChart3, Clock, Zap } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-background text-foreground selection:bg-[#00D4AA]/30" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      ...
      <div className="relative min-h-screen flex items-center justify-center overflow-hidden bg-[#060E1A]">
        <div className="absolute inset-0 z-0"><SnowRemovalHeroVideo /></div>
        ...
        <div className="relative z-20 text-center max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 mt-20">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 border border-white/20 backdrop-blur-md mb-8">
            ...
            <span className="text-sm font-medium text-white/90">❄️ Built for Snow Removal Businesses</span>
          </motion.div>
          <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }} className="text-6xl md:text-7xl lg:text-8xl font-black text-white mb-6 tracking-tight leading-[1.1]">
            Every Driveway.<br /><span className="text-[#00D4AA]">Always Clear.</span>
          </motion.h1>
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }} className="text-lg md:text-xl text-white/80 mb-10 max-w-3xl mx-auto leading-relaxed font-light">
            Certxa manages your seasonal client list, auto-notifies customers when you're on the way, handles invoicing, and keeps your route organized — even on the busiest storm days.
          </motion.p>
      ...
      <div className="bg-[#0A2540] py-6 border-y border-white/10 relative z-30">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            <p className="text-white/80 font-medium text-lg flex items-center gap-3"><span className="relative flex h-3 w-3"><span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#00D4AA] opacity-75" /><span className="relative inline-flex rounded-full h-3 w-3 bg-[#00D4AA]" /></span>Trusted by thousands of snow removal operators</p>
            <div className="flex flex-wrap justify-center gap-8 text-white/60 font-medium text-sm md:text-base">
              <div className="flex items-center gap-2"><span className="text-[#00D4AA] font-bold">✓</span> Seasonal Contract Management</div>
              <div className="flex items-center gap-2"><span className="text-[#00D4AA] font-bold">✓</span> Per-Storm Billing</div>
              <div className="flex items-center gap-2"><span className="text-[#00D4AA] font-bold">✓</span> Route Optimization</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-[#060E1A] py-20 border-b border-white/10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
            {[{ value: "4,000+", label: "Snow Operators" }, { value: "200K+", label: "Properties Serviced" }, { value: "97%", label: "Seasonal Renewal Rate" }, { value: "4.8★", label: "Average Rating" }].map((s) => (
      ...
      <div className="py-32 bg-slate-50 text-slate-900">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-20">
            <span className="inline-block px-4 py-1.5 rounded-full bg-[#00D4AA]/10 text-[#00D4AA] text-sm font-semibold mb-4">BUILT FOR SNOW REMOVAL</span>
            <h2 className="text-4xl md:text-5xl font-black mb-6 tracking-tight text-[#0A2540]">Everything Your Snow Business Needs</h2>
            <p className="text-slate-500 text-xl max-w-2xl mx-auto font-light">Manage your seasonal client list, route, and billing even during the biggest snowstorm.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[
              { icon: <Repeat className="w-6 h-6 text-[#00D4AA]" />, title: "Seasonal Client Contracts", desc: "Sign up customers for the whole season with a single booking. They're on your route for every storm, automatically. No re-booking required after every snowfall." },
              { icon: <MapPin className="w-6 h-6 text-[#00D4AA]" />, title: "Property Address & Access Notes", desc: "Store every client's address, gate codes, driveway layout, and special instructions. Your crew knows exactly what to expect before they pull up." },
              { icon: <Zap className="w-6 h-6 text-[#00D4AA]" />, title: "On-My-Way Notifications", desc: "Text clients automatically when you're heading to their property. No missed calls asking when you're coming — they know, and so do you." },
              { icon: <DollarSign className="w-6 h-6 text-[#00D4AA]" />, title: "Per-Storm or Flat-Rate Billing", desc: "Invoice per storm event or charge a seasonal flat rate. Generate and send all invoices in one batch after a storm so you get paid without the paperwork nightmare." },
              { icon: <Users className="w-6 h-6 text-[#00D4AA]" />, title: "Client Database", desc: "Your full seasonal roster in one place. See which properties are active, what package they're on, and their billing status at a glance." },
              { icon: <CheckCircle className="w-6 h-6 text-[#00D4AA]" />, title: "Job Completion Tracking", desc: "Mark each property cleared when the job is done. See your full route progress in real time and never wonder if a driveway was missed." },
              { icon: <Smartphone className="w-6 h-6 text-[#00D4AA]" />, title: "Automated Customer Communication", desc: "Clients get notifications for service start, job complete, and invoice sent. Professional communication that builds trust without adding to your workload." },
              { icon: <BarChart3 className="w-6 h-6 text-[#00D4AA]" />, title: "Season Revenue Tracking", desc: "See total earnings per storm, per week, and for the full season. Know exactly what your snow business is worth and plan next season accordingly." },
              { icon: <Clock className="w-6 h-6 text-[#00D4AA]" />, title: "Route & Schedule Planning", desc: "Plan your storm-day route through the client list, assign stops to crew members, and track progress as your team works through the neighborhood." },
      ...
      <HowItWorks steps={[
        { step: "01", title: "Load Your Client List", desc: "Add your seasonal customers with their addresses and service details. Import your existing list or add them one by one — takes minutes per client." },
        { step: "02", title: "Share Your Booking Link", desc: "Let new customers sign up for the season from your booking page. They pick their package, enter their address, and you confirm. Easy as that." },
        { step: "03", title: "Work the Storm, Get Paid", desc: "When snow hits, trigger your route, notify clients, track completions, and batch-send invoices — all from your phone while you're in the truck." },
      ]} />

      <div className="py-32 bg-slate-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-20">
            <h2 className="text-4xl md:text-5xl font-black mb-6 tracking-tight text-[#0A2540]">What Snow Operators Are Saying</h2>
            <p className="text-slate-500 text-xl max-w-2xl mx-auto font-light">Real feedback from real snow removal professionals.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <TestimonialCard quote="During a big storm I'm doing 40+ stops. Certxa keeps my whole route organized on my phone and I batch-invoice everyone at the end. Gets done in 10 minutes." name="Chad B." role="Owner, Arctic Plowing" />
            <TestimonialCard quote="The on-my-way texts to clients have basically eliminated the 'when are you coming' calls. Clients are happy, I'm less stressed. Worth every penny." name="Kevin S." role="Solo Snow Operator" />
            <TestimonialCard quote="Signing people up for seasonal contracts online changed my business. I go into each winter fully booked instead of scrambling to call people after the first storm." name="Deb T." role="Owner, Clear Path Snow Co." />
          </div>
        </div>
      </div>

      <CompareTable title="Why Snow Operators Choose Certxa" subtitle="Built for the real chaos of storm-day operations." rows={[
        ["Seasonal client contract management", true, false],
        ["Per-storm batch invoicing", true, false],
        ["On-my-way client notifications", true, false],
        ["Property access notes", true, false],
        ["Job completion tracking", true, false],
        ["Multi-crew scheduling", true, true],
        ["Online booking & payment", true, true],
        ["60-day free trial", true, false],
      ]} />

      <CTASection headline="Ready for a Fully Booked Winter?" sub="Start your 60-day free trial. No credit card required. Cancel any time." context="Join thousands of snow removal operators already using Certxa." />
      <PageFooter />
    </div>
  );
}
